...
class Main extends Component {
    ...
    trackOnClick = (duration) => {
       ...
       this.setState({ 
           loadingSatPosition: true,
           duration: duration
        });
       const urls = this.state.selected.map( sat => {
           const { satid } = sat;
           const url = `${SATELLITE_POSITION_URL}/${satid}/${observerLat}
           /${observerLong}/${observerAlt}/${endTime}/&apiKey=${SAT_API_KEY}`;
    ...
    track = () => {
        ...
        let timer = setInterval( () => {
            ...
            context2.font = "bold 14px sams-serif";
